...
export default function UpdatesManagePage() {
  ...
  const handleImageUpload = async (event) => {
    ...
    setSaving(true);
    try {
      const data = await api.admin.uploadImage(file);
      setForm((previous) => ({
        ...previous,
  ...
  const handlePdfUpload = async (event) => {
    ...
    setSaving(true);
    try {
      const data = await api.admin.uploadPdf(file);
      setForm((previous) => ({
        ...previous,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editingId) {
        await api.admin.updateUpdate(editingId, payload);
        setFeedback({ type: "success", message: "Update revised successfully." });
      } else {
        await api.admin.createUpdate(payload);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.admin.deleteUpdate(id);
      loadUpdates();
    } catch (error) {
